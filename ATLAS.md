...
├── assets/
│   └── icon.ico                   Application icon (generated by build.rs)
├── .github/
│   └── workflows/
...
| `SystemRoot` | env var (Windows) | `C:\Windows` | Base path used to locate both `Fonts\segoeui.ttf` (primary text font) and `Fonts\seguiemj.ttf` (emoji fallback font). |
